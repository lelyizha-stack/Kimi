...
function renderSummary(){
  ...
  const counts = localGames.reduce((a,i)=>{
    a[i.category] = (a[i.category] || 0) + 1;
    return a;
  },{});

  summary.innerHTML = Object.entries(counts)
    .map(([k,v]) => `<div class="mini-row"><strong>${k.toUpperCase()}</strong><span>${v} item</span></div>`)
    .join("");
}
...
function buildEntry(form){
  ...
  const platform = [...form.querySelectorAll('input[name="platform"]:checked')]
    .map(i => i.value);
  ...
  return {
    ...
    detailUrl: String(data.get("detailUrl") || "").trim() || `./posts/${category}/${slug}.html`,
    downloadUrls: {
...
function exportJson(){
  const blob = new Blob([JSON.stringify(localGames,null,2)], {type:"application/json"});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("copyEntry")?.addEventListener("click", async () => {
    if(!lastEntry) return alert("Belum ada entry baru.");
