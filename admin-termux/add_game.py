...
def ask_download_urls(platforms):
    urls = {'Windows': '', 'Android': ''}
    for platform in platforms:
        urls[platform] = ask(f'Link download {platform}', '#')
    return urls
...
def make_download_buttons(game):
    ...
    for platform in ['Windows', 'Android']:
        if platform in (game.get('platform') or []):
            ...
            buttons.append(f'<a href="{url}" class="btn btn-primary" target="_blank" rel="noopener noreferrer">{platform}</a>')
    return ''.join(buttons)

def make_genre_chips(game):
    return ''.join(f'<span class="chip">{genre}</span>' for genre in (game.get('genres') or [])[:6])

def make_platform_spans(game):
    return ''.join(f'<span class="tag platform">{p}</span>' for p in (game.get('platform') or []))

def create_detail_page(game):
    category_dir = POSTS_DIR / game['category']
    category_dir.mkdir(parents=True, exist_ok=True)
    target = category_dir / f"{game['slug']}.html"
    image_html = f'<img src="{game.get("image", "")}" alt="{game.get("title", "")}">' if game.get('image') else ''
    html = f'''<!DOCTYPE html><html lang="id"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>{game.get("title", "")} - Karbit Prime</title><link rel="stylesheet" href="../../style.css"><link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.2/css/all.min.css"></head><body><header class="site-header"><div class="container header-inner"><a href="../../" class="logo-wrap"><div class="logo-mark">KP</div><div class="logo-text"><span class="logo-title">Karbit Prime</span><span class="logo-sub">TL Indo • Mod • Cheat</span></div></a><nav class="main-nav"><a href="../../">Home</a><a href="../../donasi.html">Donasi</a><a href="../../renpy.html">Ren'Py</a><a href="../../rpgm.html">RPGM</a><a href="../../mod.html">Mod</a><a href="../../cheat.html">Cheat</a><a href="../../money-editor.html">Money Editor</a><a href="../../admin-local.html">Admin</a></nav></div></header><main class="page-shell"><div class="container"><section class="page-hero"><div class="page-copy"><div class="hero-badge">{game.get("emoji", "🎮")} Detail Game</div><h1>{game.get("title", "")}</h1><p>{game.get("description", "")}</p><div class="toolbar" style="margin-top:20px">{make_genre_chips(game)}</div></div><div class="page-icon">{game.get("emoji", "🎮")}</div></section><section class="page-section"><div class="download-card"><div class="catalog-cover">{image_html}<div class="cover-emoji">{game.get("emoji", "🎮")}</div></div><div class="download-body"><div class="post-meta"><a class="tag {game.get("category", "")}" href="../../{game.get("category", "")}.html">{str(game.get("category", "")).upper()}</a>{make_platform_spans(game)}<span class="tag status">{game.get("status", "Updated")}</span></div><h3>Informasi Ringkas</h3><p>Halaman detail ini dibuat otomatis dari script Termux.</p><div class="download-meta"><div><strong>{game.get("version", "-")}</strong><span>Versi</span></div><div><strong>{game.get("size", "-")}</strong><span>Ukuran</span></div><div><strong>{game.get("language", "-")}</strong><span>Bahasa</span></div></div><div class="note-box" style="margin-top:18px">Genre: {', '.join(game.get("genres", []))}<br>Platform: {', '.join(game.get("platform", []))}<br>Status: {game.get("status", "Updated")}</div><div class="download-actions" style="margin-top:18px">{make_download_buttons(game)}<a href="../../{game.get("category", "")}.html" class="btn btn-secondary">Kembali ke Katalog</a></div></div></div></section></div></main></body></html>'''
...
def main():
    ...
    slug = slugify(title)
    game = {'id': make_id(games, category), 'slug': slug, 'title': title, 'category': category, 'genres': genres, 'platform': platform, 'version': version, 'size': size, 'language': language, 'status': status, 'emoji': emoji, 'image': image, 'description': description, 'detailUrl': f'./posts/{category}/{slug}.html', 'downloadUrls': download_urls, 'createdAt': date.today().isoformat()}
    games.insert(0, game)
    save_games(games)
